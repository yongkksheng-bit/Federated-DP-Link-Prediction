...
- Evidence: **F1**. The complete 21-page publisher PDF supplied locally by the researcher was inspected and hashed in `P0_RESTRICTED_EVIDENCE_REGISTER.md`.
- Verified ownership and architecture: the recommendation platform/server already owns the raw user-item interaction graph. Separate clients own auxiliary user-user or item-item relation graphs, perturb those graphs once, and upload them. The server then constructs a global heterogeneous graph and performs all subsequent HGNN training centrally. The authors explicitly contrast this one-shot publication design with conventional multi-round federated learning. PP-HGRL is therefore decentralized multi-owner graph publication followed by centralized training, not federated optimization over client-held interaction edges.
- Verified task and evaluation: recommendation is formulated as latent user-item link prediction, but experiments report sampled implicit-feedback Recall and NDCG on Ciao, Douban-Book, Yelp, and Epinions. The paper does not define intra-/cross-client candidate links because the prediction relation is held by the server rather than partitioned across clients.
- Verified privacy mechanisms: PGDP applies randomized response, noisy degree estimation, and post-processed adaptive edge sampling to auxiliary adjacency lists and claims epsilon edge-DP. GEP adds Gaussian noise to layer-wise embeddings computed from the raw server-owned interaction graph. Its sensitivity proof assumes row-normalized input embeddings and bounds one graph-convolution query by one; the paper then linearly composes the per-layer/per-batch RDP cost over training.
- Verified output boundary: the text invokes post-processing to claim that the trained model output is edge-DP. It does not specify or account for an unrestricted pair-score transcript, a bounded recommendation API, or embeddings recomputed by rereading a private graph at deployment. Thus "from data sharing to model deployment" means perturbed graph publication plus model release under the stated mechanism, not a formal theorem for arbitrary recommendation queries.
- Audit concern: the paper combines PGDP and GEP using parallel composition because they operate at different "data stages." When both mechanisms depend on the same underlying relation, different processing stages alone do not establish disjoint-record parallel composition; sequential composition would normally be required. In its instantiated ownership model, PGDP protects auxiliary client relations while GEP protects the server interaction relation, which may make a max-style composition defensible only after the protected datasets and neighboring relation are stated separately. This ambiguity prevents importing its headline epsilon as a matched guarantee without re-derivation.
- Relation: **mandatory highest-risk near predecessor and baseline**. It rules out novelty based on decentralized edge-DP recommendation, heterogeneous private graph publication, or a generic deployment claim. A surviving contribution must instead address ordinary homogeneous interaction edges distributed across clients, multi-round transcript accounting, cross-client candidate links, and an explicitly private score output.
...
- Evidence: **F1**. The complete eight-page IEEE proceedings PDF supplied locally by the researcher was inspected and hashed in `P0_RESTRICTED_EVIDENCE_REGISTER.md`.
- Verified setting: CF-DPGNN centrally constructs one user-item bipartite interaction graph and treats recommendation as link prediction. There are no clients, distributed edge owners, federated rounds, or aggregation protocol.
- Verified mechanism: a popularity-aware procedure samples r-hop training subgraphs. Gradients are clipped per sampled subgraph, summed over a subgraph minibatch, perturbed once with Gaussian noise, and used for a centralized update. The paper defines neighboring graphs by one edge and gives an RDP amplification expression based on the random maximum multiplicity of a node across sampled subgraphs, followed by linear composition over iterations.
- Verified output and experiments: Algorithm 3 outputs model parameters; recommendation scores are inner products of learned embeddings. The paper does not separately formalize the privacy of a score-query transcript or state whether inference recomputes embeddings from the raw graph. Experiments use MovieLens-1M, Gowalla, and Yelp, Recall/NDCG, ten runs, epsilon 5, and a confidence-based membership-inference diagnostic.
- Audit concern: clipping is per sampled subgraph rather than visibly per interaction edge, while one private edge may influence several overlapping sampled subgraphs. The claimed accountant attempts to handle overlap through a random multiplicity variable, but it is not a drop-in substitute for an independently audited per-edge DP-SGD accountant. Any reproduction must implement and verify the exact sampler and multiplicity bound.
- Relation: mandatory **centralized** edge-DP recommendation baseline. It rules out novelty based on edge-DP GNN recommendation or subgraph-amplified DP-SGD, but it does not solve federated ownership, complete client-transcript privacy, or cross-client LP.
...
1. Trace forward citations of Solitude, LGA-PGNN, DPLP, GAP, PDGL, PP-HGRL, CF-DPGNN, FKGE, and FedLink in at least two independent indexes immediately before submission.
2. Re-run the frozen query families immediately before freezing P1 and again before manuscript submission.
3. Re-derive PP-HGRL's composition and CF-DPGNN's overlap-aware accountant before using either epsilon value in a matched experiment.
