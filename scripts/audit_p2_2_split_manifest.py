"""Audit P2.2 development files and sealed hashes without test decrypt."""
...
MANIFEST = ROOT / "data" / "manifests" / "p2_2_split_manifest.json"
PROCESSED = ROOT / "data" / "processed" / "p2_2_confirmatory"
SEALED = ROOT / "data" / "sealed" / "p2_2_confirmatory"
OUTPUT = ROOT / "data" / "manifests" / "p2_2_split_audit.json"
...
def main() -> None:
    if OUTPUT.exists():
        raise SystemExit("P2.2 split audit exists; refusing overwrite")
    ...
    for dataset in config["datasets"]:
        ...
        if len(record["public_cell_counts"]) != config["public_coarsening"]["cells"]:
            failures.append(f"{dataset}: wrong public cell count")
        if min(record["public_cell_counts"]) < 2:
            failures.append(f"{dataset}: public cell too small")
        for split in record["splits"]:
            ...
            hash_match = sha256(sealed) == split["commitments"]["sealed_payload_sha256"]
            if not hash_match:
    ...
    if failures:
        raise SystemExit("P2.2 split audit failed")
