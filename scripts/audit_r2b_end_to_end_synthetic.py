"""Replay and audit the complete frozen R2B synthetic experiment."""
...
from run_r2b_end_to_end_synthetic import generate_records, summarize
...
def main() -> None:
    ...
    expected_records = (
        len(config["graph"]["regimes"])
        * config["graph"]["seeds"]
        * len(config["training_branch"]["epsilon_grid"])
        * len(config["certification"]["epsilon_grid"])
        * len(config["certification"]["visibility_models"])
    )
    checks = {
        "record_count_complete": len(stored) == expected_records,
    ...
    if audit["status"] != "PASS":
        raise SystemExit("R2B audit failed")
