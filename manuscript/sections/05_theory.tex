...
\end{equation}
On the registered role-labelled database, the conservative sequential bound
$\rho_{\mathrm{joint}}\leq\rho_T+\rho_C$ is also valid without taking
parallel-composition credit. This sum does not by itself establish privacy
under adjacency of the raw graph before role assignment.
\end{theorem}

The proof conditions on the one role-labelled record containing the
neighboring edge.
Post-processing handles certification after a training-edge change, while
uniform conditional DP handles a certification-edge change. R5 reports the
sequential sum under its registered role-labelled adjacency because its
historical benchmark split was not designed to justify a stronger raw-graph
partition theorem.

\subsection{Finite-population no-harm certificate}

Let $H$ be the sealed positive holdout of size $N$, fixed independently of the
assignment outputs, and
\begin{equation}
 \Delta_H(R)=\frac{1}{N}\sum_{e\in H}d_R(e).
\end{equation}
In the ideal theorem, independent Bernoulli bits assign each $e\in H$ to
certification with probability $p_C$; $Q5$ is the complement. R5 instantiates
these bits with an edge-keyed SHA-256 map under the registered random-oracle
model. A production protocol can instead use a secret-key PRF whose key is
hidden from data providers until $H$ is fixed, yielding a computational
analogue. Neither interpretation is an unconditional information-theoretic
claim about deterministic public SHA-256 outputs.

\begin{theorem}[Finite-holdout no-harm]
\label{thm:noharm}
Assume that, conditional on the frozen holdout $H$, its certification
assignment bits are independent of the training release $R$. Conditional on
$(R,H,n_C)$, the certification subset is then a simple random sample without
replacement. With the certificate in \cref{eq:certificate},
\begin{equation}
\Pr\!\left(A=1\ \wedge\ \Delta_H(R)<\gamma
           \mid R,H,n_C\right)
...
edges. Under the stated ideal randomization, or computationally under its
implementation contract, conditional on the realized bounded values and
certification count, shared graph endpoints do not alter the random-sampling
argument. A one-sided Serfling bound
...
The same bound conditional on $(R,H)$ follows by averaging over $n_C$;
counts that fail the registered validity checks force abstention.
The displayed failure probability is statistical in the ideal
Bernoulli/random-oracle model. A standard-model PRF instantiation adds the
PRF distinguishing advantage to that bound.
...
\label{thm:lower}
Let $-1<\gamma<1$, $0<\alpha\leq1-\gamma$, and let the positive integer
$\chi$ divide $n$. For block-replicated Bernoulli hard instances with
dependence $\chi$, every central pure $\eps$-DP selector with fixed testing
error $0<\eta<0.45$ requires, up to constants,
\begin{equation}
...
For approximate DP, \cref{app:lower} gives the exact privacy term
$2D_\star/\alpha$, which is nontrivial when $D_\star>0$ and reduces to the
displayed inverse-$\eps$ dependence only when the $\delta$ term is
non-dominant. Thus the sufficient and necessary boundaries match in their
principal dependence for pure DP and sufficiently small $\delta$, not
uniformly over all approximate-DP regimes. This does not make the specific
Gaussian certificate minimax optimal in $\delta$, nor does it prove that
arbitrary graph edges follow the hard-instance model. The lower bound uses the
private Le Cam inequality of \cite{acharya2021private}.
