...
\end{equation}
An edge-keyed SHA-256 map assigns each $e\in H$ to certification with
probability $p_C$; $Q5$ is the complement.  The random-sampling statement
below is conditional on the registered random-oracle (or equivalently a
secret-key PRF) interpretation of this map.  It is not an unconditional
information-theoretic claim about deterministic SHA-256 outputs.
...
edges. Under the stated pseudorandomness assumption, conditional on the
realized bounded values and certification count, shared graph endpoints do
