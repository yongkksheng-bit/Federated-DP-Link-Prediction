...
Poisoning-based link-inference attacks further show that observable federated
outputs can expose cross-client structure \cite{yang2026poisoning}.
...
CE-FedGNN protects cross-client representation exchange under metric DP, a
different privacy object from ordinary-edge DP \cite{guo2026cefedgnn}.
...
\begin{itemize}
  ...
  prediction under registered role-labelled add/remove-edge adjacency,
  complete server-transcript accounting, cross-client candidates, and
  inference-closed scoring.
  ...
  and a pure central-DP testing lower bound with matching principal dependence
  $\chi/\alpha^2+1/(\eps\alpha)$ up to constants; approximate DP uses an
  explicit $D_\star$ term.
